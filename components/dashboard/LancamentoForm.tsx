  ...
  if (!open) {
    return (
      <button
        ...
        className="text-xs font-sans text-primary hover:text-primary-hover transition-colors px-2 py-1 rounded-lg hover:bg-primary/10"
      >
        {comissaoAtual > 0 ? `Atualizar (${formatBRL(comissaoAtual)})` : 'Lançar'}
        {sucesso && <span className="ml-2 text-green-400">✓</span>}
      </button>
    )
  }
