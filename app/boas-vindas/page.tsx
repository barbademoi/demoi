...
export default async function BoasVindasPage({ searchParams }: Props) {
  ...
  return (
    <Layout>
      {!usuario ? (
        <Processando email={email} transaction={transaction} />
      ) : usuario.senha_definida ? (
        <JaDefinida />
      ) : (
        <FormCard email={email} transaction={transaction} />
      )}
    </Layout>
  )
}
...
function FormCard({ email, transaction }: { email: string; transaction: string }) {
  return (
    <div className="card p-8 space-y-6">
      <div>
        ...
        <p className="mt-2 text-xs text-text-muted font-sans truncate">
          {email}
        </p>
      </div>
...
function Processando({ email, transaction }: { email: string; transaction: string }) {
  return (
...
function LinkInvalido() {
  return (
    <div className="card p-8 text-center space-y-4">
      ...
      <p className="text-text-muted text-sm font-sans">
        Este link não é válido. Se acabou de comprar, verifique o email de confirmação
        ou entre em contato com o suporte.
      </p>
      <a
        href="mailto:suporte@barbermeta.com.br"
        className="text-sm text-[#D4A85A] hover:text-white transition-colors font-sans"
      >
