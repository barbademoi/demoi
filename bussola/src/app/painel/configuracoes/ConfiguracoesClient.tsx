...
interface ToggleDef {
  chave: 'categorizacao_auto' | 'resumo_semana'
  label: string
}

const TOGGLES: ToggleDef[] = [
  { chave: 'categorizacao_auto', label: 'Categorização automática de observações' },
...
export default function ConfiguracoesClient({ inicial }: { inicial: ConfigIA }) {
  ...
  return (
    <div className="card p-5 space-y-6">
          ...
          {TONS.map((t) => (
            <label
              ...
              className={[
                ...
                config.tom === t.v ? 'border-marrom bg-linho' : 'border-border hover:border-marrom/40',
              ].join(' ')}
            >
              <input type="radio" checked={config.tom === t.v} onChange={() => setConfig((c) => ({ ...c, tom: t.v }))} className="accent-marrom mt-0.5" />
              <span>
                <span className="block text-sm font-medium text-text">{t.label}</span>
                <span className="block text-xs text-chumbo">{t.desc}</span>
              </span>
            </label>
          ))}
        </div>
      ...
      <div className="space-y-2">
        {TOGGLES.map((t) => (
          <label key={t.chave} className="flex items-center justify-between gap-3 p-3 rounded-md border border-border">
            <span className="text-sm text-text">{t.label}</span>
            <input
              type="checkbox"
              checked={config[t.chave]}
              onChange={(e) => setConfig((c) => ({ ...c, [t.chave]: e.target.checked }))}
              className="accent-marrom w-5 h-5"
            />
          </label>
        ))}
      </div>
